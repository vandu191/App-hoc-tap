...
import { Menu, Ship, Smartphone, X, Download } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Lắng nghe sự kiện cài đặt của trình duyệt (chỉ Android/Chrome hỗ trợ)
  useEffect(() => {
    window.addEventListener('beforeinstallprompt', (e) => {
      e.preventDefault();
      setDeferredPrompt(e);
      // Chỉ hiện banner nếu chưa cài đặt
      if (!window.matchMedia('(display-mode: standalone)').matches) {
        setShowInstallBanner(true);
      }
    });

    // Kiểm tra nếu là iOS để hiện hướng dẫn riêng
  ...
  const handleInstallApp = async () => {
    if (deferredPrompt) {
      ...
      // Hướng dẫn cho iOS hoặc các trình duyệt khác
        ...
        <div className="fixed top-20 left-4 right-4 z-[60] bg-[#004a99] text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between border border-white/20 animate-in slide-in-from-top-10">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="bg-white text-[#004a99] px-4 py-2 rounded-xl text-xs font-bold active:scale-95 transition-transform shadow-lg"
            >
              Cài đặt
            </button>
            <button onClick={() => setShowInstallBanner(false)} className="p-1 opacity-60">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
